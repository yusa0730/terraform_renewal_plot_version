...
resource "aws_route_table" "private" {
  ...
  tags = {
    Name = "${local.project_name}-${local.env}-private"
  }
}
...
resource "aws_vpc_endpoint" "from_api_gateway_to_lambda" {
  ...
  tags = {
    Name = "vpce-from-api-gateway-to-lambda"
  }
}
...
resource "aws_vpc_endpoint" "from_lambda_to_dynamodb" {
  ...
  tags = {
    Name = "vpce-from-lambda-to-dynamodb"
  }
}

resource "aws_vpc_endpoint_route_table_association" "from_lambda_to_dynamodb" {
  route_table_id  = aws_route_table.private.id
